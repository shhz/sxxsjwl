(function($) {
	$.fn.poposlides = function(options) {
		...
		return this.each(function() {
			...
			var $this = $(this), slide = $this.children(), index = 0;
			len = slide.length - 1, slideWidth = $this.width(),
					prev = settings.prev, next = settings.next;
			...
			pagnation = function() {
				...
				for (var i = 1; i <= len + 1; i++) {
					paginationStr += "<li><a href='javascript:void(0)'>" + i
							+ "</a>";
				}
			...
			// 是否需要左右导航图标
			if (settings.nav) {
				var navStr = "<a href='javascript:void(0)' class="
						+ prev.substring(1) + "></a>"
